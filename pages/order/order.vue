<template>
	<view>
		<!-- 导航栏 -->
		<u-navbar :background="{background: 'linear-gradient(to right, #00baad 0%, #7948ea 95%)'}"
			:border-bottom="false" title="我的订单" title-color="#fff" back-icon-color="#fff"></u-navbar>
		<view class="wrap">
			...
			gotoOrderDetail(obj) {
				uni.navigateTo({
					...
					success() {
						setTimeout(() => {
							// 使用JSON正反序列化传输非响应式数据
							uni.$emit('gotoDetail', JSON.parse(JSON.stringify(obj)))
						}, 1000)
					}
				})
			}
